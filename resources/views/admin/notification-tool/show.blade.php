...
@section('page-title', 'Notification Log Details')
...
@php
    ...
    $studentIds = is_array($communicationLog->student_ids) ? $communicationLog->student_ids : [];
@endphp

<div class="mb-5 flex flex-wrap items-center justify-between gap-3">
    <a href="{{ route('admin.notification-tool.index') }}"
       class="inline-flex items-center gap-2 text-sm font-medium text-gray-600 transition hover:text-green-700">
        <i class="fas fa-arrow-left text-xs"></i>
        Back to Notification Tool
    </a>
    @if($schedule)
        <a href="{{ route('admin.class-schedules.show', $schedule) }}#notify-enrolled"
           class="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-700 shadow-sm transition hover:border-green-300 hover:text-green-700">
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div class="flex flex-wrap items-start justify-between gap-4 border-b border-gray-100 bg-gray-50/80 px-5 py-4 sm:px-6">
            <div class="min-w-0">
                ...
                <p class="mt-1 text-sm text-gray-500">Class ID: #{{ $communicationLog->class_schedule_id }}</p>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold
                    {{ $communicationLog->failed_count > 0 ? 'bg-amber-50 text-amber-800' : 'bg-green-50 text-green-800' }}">
        ...
        <div class="grid grid-cols-1 gap-0 sm:grid-cols-3">
            ...
            <div class="border-b border-gray-100 px-5 py-4 sm:border-b-0 sm:border-r sm:px-6">
                <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Delivery method</p>
                <p class="mt-1 text-sm font-semibold uppercase text-gray-900">{{ $communicationLog->delivery_method }}</p>
            </div>
            <div class="px-5 py-4 sm:px-6">
                <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Student IDs</p>
    ...
    <div class="grid grid-cols-1 gap-5 lg:grid-cols-5">
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm lg:col-span-3">
            <div class="flex items-center gap-2 border-b border-gray-100 px-5 py-3.5 sm:px-6">
                <span class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-green-50 text-green-700">
                    <i class="fas fa-comment-alt text-sm"></i>
                </span>
                <h4 class="text-sm font-bold text-gray-900">Message content</h4>
            </div>
                ...
                <div class="max-h-[28rem] overflow-y-auto">
                    <table class="min-w-full">
                        <thead class="sticky top-0 bg-gray-50">
                            <tr>
                                <th class="px-5 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wide text-gray-400 sm:px-6">ID</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach($students as $student)
                                <tr class="hover:bg-gray-50/80">
                                    <td class="px-5 py-3 text-sm font-semibold text-gray-700 sm:px-6">#{{ $student->id }}</td>
                                    <td class="px-5 py-3 text-sm font-medium text-gray-900 sm:px-6">{{ $student->name }}</td>
                                    <td class="px-5 py-3 text-sm text-gray-500 sm:px-6">{{ $student->email }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
