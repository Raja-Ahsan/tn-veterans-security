...
<div class="mb-5">
    ...
    <p class="mt-1 text-sm text-gray-500">{{ $category->name }} · <span class="font-mono text-xs">{{ $category->slug }}</span></p>
</div>

<form method="POST" action="{{ route('admin.categories.update', $category) }}" class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
    @csrf
    @method('PUT')
