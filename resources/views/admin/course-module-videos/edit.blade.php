...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
    <div>
        <h3 class="text-xl font-semibold text-gray-900">Edit {{ $courseModuleVideo->displayTitle() }}</h3>
        <p class="mt-1 text-sm text-gray-500">Module: {{ $courseModule->title }}</p>
    </div>
...
<form method="POST" action="{{ route('admin.classes.course-modules.videos.update', [$service, $courseModule, $courseModuleVideo]) }}" enctype="multipart/form-data" class="w-full space-y-6 pb-8">
    @csrf
    @method('PUT')
