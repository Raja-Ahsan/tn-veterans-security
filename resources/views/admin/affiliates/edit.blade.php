...
<div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <div>
        ...
        <h3 class="mt-2 text-xl font-semibold text-gray-900">Edit affiliate</h3>
        <p class="mt-1 text-sm text-gray-500">{{ $affiliate->name }}</p>
    </div>
</div>

<form method="POST" action="{{ route('admin.affiliates.update', $affiliate) }}" class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
    @csrf
    @method('PUT')
