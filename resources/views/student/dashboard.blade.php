...
@if(($guardForms ?? collect())->isNotEmpty())
<div class="mb-8 rounded-xl border border-emerald-100 bg-white p-5 shadow-sm sm:p-6">
    <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-lg font-bold text-gray-900">Your completion forms</h2>
            <p class="mt-0.5 text-sm text-gray-500">State Form IN-1144 — view or download anytime.</p>
        </div>
        <a href="{{ route('student.certificates.index') }}" class="text-sm font-semibold text-[var(--brand)] hover:text-[var(--brand-dark)]">
            All certificates <i class="fas fa-arrow-right ml-1 text-xs"></i>
        </a>
    </div>
    <div class="space-y-3">
        @foreach($guardForms as $form)
            <div class="flex flex-col gap-3 rounded-xl border border-emerald-100 bg-emerald-50/40 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    <h3 class="font-semibold text-gray-900">Certificate of Successful Completion</h3>
                    <p class="text-sm text-gray-600">{{ $form->form_number }}@if($form->service) · {{ $form->service->title }}@endif</p>
                </div>
                <a href="{{ route('student.guard-training-forms.show', $form) }}" target="_blank"
                   class="inline-flex items-center justify-center gap-2 rounded-lg bg-[var(--brand)] px-4 py-2 text-sm font-semibold text-white hover:bg-[var(--brand-dark)]">
                    <i class="fas fa-download text-xs"></i> View / Download
                </a>
            </div>
        @endforeach
    </div>
